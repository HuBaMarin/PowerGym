package com.amarina.powergym.ui.profile
...
class ProfileViewModel(
    ...
    private fun loadUserProfile() {
        ...
        viewModelScope.launch {
            ...
            userRepository.getUserById(userId)
                ...
                .onFailure { exception ->
                    _profileState.value = ProfileState.Error(exception.message ?: "Error desconocido")
                }
        }
    }
    ...
    fun saveUserProfile() {
        ...
        if (currentState is ProfileState.Success) {
            viewModelScope.launch {
                ...
                userRepository.updateUser(currentState.user)
                    .onSuccess {
                        _updateState.value = UpdateState.Success
                        sessionManager.saveUserSession(currentState.user)
                        _navigationEvents.emit(NavigationEvent.Back)
                    }
                    .onFailure { exception ->
                        _updateState.value = UpdateState.Error(exception.message ?: "Error desconocido")
                    }
            }
        }
    }
    ...
    fun deleteUserAccount() {
        ...
        if (currentState is ProfileState.Success) {
            viewModelScope.launch {
                userRepository.deleteUser(currentState.user)
                    .onSuccess {
                        sessionManager.logout()
                        _navigationEvents.emit(NavigationEvent.ToLogin)
                    }
                    .onFailure { exception ->
                        _updateState.value = UpdateState.Error(exception.message ?: "Error al eliminar cuenta")
                    }
            }
        }
    }
